...
////////////////////////////////////////////////////
/////////////////Платформа(игрок)///////////////////
procedure TMyArcanoid.SetPlayerX(Value:Integer);//Задание новой координаты платформы
begin
...
procedure TMyArcanoid.WMLBttonDown(var M: Tmessage);//Нажатие кнопки мыши
var Mes: TMessage;
begin
...
procedure TMyArcanoid.WMLBttonUp(var M: Tmessage);//Отжатие кнопки мыши
var Mes1: TMessage;
begin
  inherited;
  FOnMove:=False;
end;
procedure TMyArcanoid.WMMove(var M: TMessage);//Перемещение мыши
begin
...
procedure TMyArcanoid.SetPlayerColor(Value: TColor);//Установка цвета игрока
begin
...
////////////////////////////////////////////////////
////////////////////////Мяч/////////////////////////
procedure TMyArcanoid.SetBallX(Value:Real);//Установка координаты мяча по оси X
begin
...
procedure TMyArcanoid.SetBallY(Value:Integer);//Установка координаты мяча по оси Y
begin
...
procedure TMyArcanoid.BallMove;//Перемещение мяча
begin
    if (FBallSpeedX<=0)and(FBallX-FBallRadius<=0) then//Столкновение с левой стенкой
    begin
      FBallSpeedX:=FBallSpeedX*-1;
    end else
    begin
      if (FBallSpeedX>=0)and(FBallX+FBallRadius>=Width) then//Столкновение с правой стенкой
      begin
        FBallSpeedX:=FBallSpeedX*-1;
      end;
    end;
    if (FBallY-FBallRadius<=0) then//Столкновение с верхней стенкой
    begin
      FBallSpeedY:=FBallSpeedY*-1;
    end else
    begin
      //Сталкновение с платформой
      ...
      begin
        FBallSpeedY:=FBallSpeedY*-1;
        if (FBallX<FPlayerX+FPlayerWidth/2) then//Столкновние с левой частью платформы
        begin
          FBallSpeedX:=(((FPlayerX+FPlayerWidth/2)-FBallX)/10)*-1;
        end else
        begin
          FBallSpeedX:=(FBallX-(FPlayerX+FPlayerWidth/2))/10;//Столкновние с правой частью платформы
        end;
      end;
    end;
...
procedure TMyArcanoid.SetBallColor(Value: TColor);//Изменение цвета мяча
begin
...
////////////////////////////////////////////////////
///////////////////////Плитки///////////////////////
function TMyArcanoid.BallAndPlatformsConflict:Boolean;//Столкновение мяча с плиткой
var
  i,j:Integer;
begin
  for i:=0 to M-1 do
...
procedure TMyArcanoid.SetPlatformsColor(Value: TColor);//Изменение цвета плиток
begin
...
////////////////////////////////////////////////////
////////////////////////Игра////////////////////////
procedure TMyArcanoid.StartPositionPlayer;//Возвращение игровых элементов в начальное положение
var
  i,j:integer;
begin
...
function TMyArcanoid.CountFalsePlatforms:Integer;//Счётчик сбитых плиток
var
  i,j,count:integer;
begin
  count:=0;
...
////////////////////////////////////////////////////
//////////////////////Таймер////////////////////////
destructor TMyArcanoid.Destroy;//Деструктор таймера
begin
   FTimer.Free;
   inherited;
end;
procedure TMyArcanoid.StartArcanoid;//Запуск игры
begin
  FTimer.Enabled:=True;
  DoScore;
end;
procedure TMyArcanoid.DoTimer(Sender: TObject);//Таймер
begin
